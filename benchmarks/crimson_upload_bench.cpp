...
BenchmarkResult run_crimson_upload_benchmark(const BenchmarkRunConfig& config)
{
    std::vector<std::array<float, 18>> vertices;
    std::vector<std::array<std::uint32_t, 3>> indices;
    const std::vector<crimson::RenderMeshUploadDesc> uploads =
    ...
    return run_benchmark("crimson_upload_tracker", config, [&]() {
        ...
        const auto first = tracker.process_uploads(uploads);
        const auto clean = tracker.process_uploads(uploads);
        crimson::FrameUploadStats upload_stats;
        if (first) {
            upload_stats.mesh_create_count += first.value().mesh_create_count;
            upload_stats.uploaded_vertex_bytes += first.value().uploaded_vertex_bytes;
            upload_stats.uploaded_index_bytes += first.value().uploaded_index_bytes;
        }
        if (clean) {
            upload_stats.skipped_clean_resource_count += clean.value().skipped_clean_resource_count;
        }
