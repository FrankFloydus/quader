...
[[nodiscard]] IoError make_mapping_error(std::string message, std::string detail, std::string subject)
{
    if (!detail.empty()) {
        message += " ";
        message += detail;
...
{
    const bool has_unsupported_occlusion = source.occlusion_texture.has_value() || source.occlusion_strength != 1.0F;
    if (!has_unsupported_occlusion) {
        return;
    }
...
{
    const crimson::BaseShaderId base_shader_id = base_shader_for_alpha_mode(source.alpha_mode);
    const crimson::BaseShaderDefinition* definition = registry.find(base_shader_id);
    if (definition == nullptr) {
        return quader::foundation::Result<GltfMaterialMapping, IoError>::failure(make_mapping_error(
            ...
            std::string(crimson::base_shader_id_name(base_shader_id)),
            source.name));
    }
    ...
    const auto validation = crimson::validate_material_instance(registry, mapping.material);
    if (!validation) {
        IoError error = make_mapping_error(
            "Crimson rejected mapped glTF material.",
            validation.error().message + " " + validation.error().detail,
            source.name);
