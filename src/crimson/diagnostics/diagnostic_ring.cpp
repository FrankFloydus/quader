...
DiagnosticRing::DiagnosticRing(std::size_t capacity) : capacity_(capacity) {
...
DiagnosticRing make_diagnostic_ring_from_recent(
	...
	const std::size_t kFirst = diagnostics.size() > capacity ? diagnostics.size() - capacity : 0;
	for (std::size_t index = kFirst; index < diagnostics.size(); ++index) {
