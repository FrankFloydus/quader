...
{
    const auto it = std::find_if(resources_.begin(), resources_.end(), [name](const RenderResourceRecord& record) {
    ...
    for (const RenderPass& pass : passes_) {
        ...
        for (const ResourceUse& use : pass.resources) {
            ...
            if (contains_name(resources_used_by_pass, use.resource_name)) {
                return quader::foundation::Result<void, std::string>::failure(
                    "Render pass '" + pass.name + "' references resource '" + use.resource_name
                    + "' more than once.");
            }
            insert_name(resources_used_by_pass, use.resource_name);

            const bool was_written = contains_name(written_resources, use.resource_name);
            if (use.access == RenderResourceAccess::Read && !was_written) {
                return quader::foundation::Result<void, std::string>::failure(
                    "Render pass '" + pass.name + "' reads resource '" + use.resource_name
                    + "' before it is written.");
            }
            if (use.access == RenderResourceAccess::Write && was_written) {
                return quader::foundation::Result<void, std::string>::failure(
                    "Render pass '" + pass.name + "' writes resource '" + use.resource_name
                    + "' after it was already written; use ReadWrite for ordered modification.");
            }
