...
[[nodiscard]] GridPlane grid_plane_for_camera(const RenderCamera& camera)
{
    ...
    const float abs_x = std::abs(forward.x);
    const float abs_y = std::abs(forward.y);
    const float abs_z = std::abs(forward.z);
...
GridOverlayCommand make_grid_overlay_for_view(const RenderView& view)
{
    ...
    const GridPlane plane = grid_plane_for_camera(camera);
    const float aspect = std::max(0.0001F, static_cast<float>(std::max<std::uint16_t>(1, view.rect.width))
    ...
    if (camera.projection == CameraProjection::Orthographic) {
        const float plane_height = std::max(0.01F, camera.orthographic_height_m) * kViewportGridOrthographicMargin;
        if (plane == GridPlane::YZ) {
            command.plane_width = plane_height;
            command.plane_height = plane_height * aspect;
        } else {
            command.plane_width = plane_height * aspect;
            command.plane_height = plane_height;
        }
    }
