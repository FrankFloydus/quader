...
FrameInstancingStats make_instancing_stats(std::span<const InstanceBatch> batches) noexcept
{
    ...
    for (const InstanceBatch& batch : batches) {
        const std::uint32_t instance_count = static_cast<std::uint32_t>(batch.instances.size());
        stats.submitted_instance_count += instance_count;
        if (instance_count > 1) {
            ++stats.instanced_batch_count;
            stats.saved_draw_call_count += instance_count - 1;
        } else {
