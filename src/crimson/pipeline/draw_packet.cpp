...
{
    const RenderQueue queue = object.queue == RenderQueue::PrototypeOpaque
        ? RenderQueue::Opaque
        : object.queue;
    ...
    float view_aspect_ratio)
{
    ...
    const CameraFrustum frustum = make_camera_frustum(camera, view_aspect_ratio);
    for (const RenderObject& object : objects) {
        ...
        const CullingDecision culling = cull_object_bounds(frustum, object.world_bounds);
        if (culling.invalid_bounds) {
        ...
        const RenderMaterialHandle material = material_or_fallback(materials, object.material, fallback_material);
        const BaseShaderDefinition* definition = definition_for_object(registry, materials, object, material);
        if (definition == nullptr) {
            continue;
        }
        if (definition->domain != RenderDomain::LitSurface
            && definition->domain != RenderDomain::TransparentSurface) {
            continue;
        }

        DrawPacket packet = packet_from_object(object, *definition, material, fallback_mesh, camera);
        switch (packet.queue) {
        case RenderQueue::Opaque:
            result.opaque.push_back(std::move(packet));
            break;
        case RenderQueue::AlphaCutout:
            result.alpha_cutout.push_back(std::move(packet));
            break;
        case RenderQueue::Transparent:
            result.transparent.push_back(std::move(packet));
            break;
