...
[[nodiscard]] bool descriptors_match(const RenderResourceDesc& left, const RenderResourceDesc& right) noexcept
{
    return left.name == right.name
        ...
        && left.extent.width_px == right.extent.width_px
        && left.extent.height_px == right.extent.height_px
        && left.extent.device_pixel_ratio == right.extent.device_pixel_ratio
        && left.external == right.external
        && left.resize_dependent == right.resize_dependent
...
{
    std::uint64_t flags = BGFX_TEXTURE_RT
...
{
    const auto it = std::find_if(targets.begin(), targets.end(), [name](const GpuFrameTargetRecord& record) {
...
{
    const std::uint64_t graph_generation = graph.resize_generation();
    ...
    for (const RenderResourceRecord& graph_resource : graph.resources().resources()) {
        const auto existing = std::find_if(targets_.begin(), targets_.end(), [&](const GpuFrameTargetRecord& record) {
        ...
        GpuFrameTargetRecord record = std::move(*existing);
        if (!descriptors_match(record.desc, graph_resource.desc)
            || record.graph_generation != graph_resource.generation) {
            record.desc = graph_resource.desc;
            record.graph_generation = graph_resource.generation;
    ...
    graph_resize_generation_ = graph_generation;
}

bool GpuFrameResources::synchronize(const RenderGraph& graph, RendererStatus& status)
{
    const std::uint64_t previous_generation = graph_resize_generation_;
    synchronize_descriptors(graph);
    const bool generation_changed = previous_generation != graph_resize_generation_;
...
{
    const auto it = std::find_if(targets_.begin(), targets_.end(), [name](const GpuFrameTargetRecord& record) {
        return record.desc.name == name;
    });
...
bool GpuFrameResources::create_runtime_targets(RendererStatus& status)
{
    ...
    for (GpuFrameTargetRecord& record : targets_) {
        ...
        const std::optional<bgfx::TextureFormat::Enum> format = bgfx_texture_format(record.desc.format);
        if (!format) {
        ...
        const bgfx::TextureHandle attachment = record.texture.get();
        record.framebuffer.reset(bgfx::createFrameBuffer(1, &attachment, false));
...
bool GpuFrameResources::recreate_composite_framebuffers(RendererStatus& status)
{
    ...
    if (!hdr_scene_framebuffer_.valid()) {
        const std::array<bgfx::TextureHandle, 2> targets = {hdr->texture.get(), depth->texture.get()};
    ...
    if (!tone_mapped_framebuffer_.valid()) {
        const std::array<bgfx::TextureHandle, 2> targets = {tone->texture.get(), depth->texture.get()};
    ...
    if (!picking_framebuffer_.valid()) {
        const std::array<bgfx::TextureHandle, 2> targets = {picking->texture.get(), depth->texture.get()};
    ...
    const bool ok = hdr_scene_framebuffer_.valid()
        && tone_mapped_framebuffer_.valid()
        && picking_framebuffer_.valid();
    if (!ok) {
        push_frame_resource_error(
            status,
            RendererDiagnosticCode::FrameBufferUnsupported,
