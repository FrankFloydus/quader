...
{
    const ColorLinear linear = srgb_to_linear(ColorSrgb{color.r, color.g, color.b, color.a});
...
{
    const std::uint64_t flags = BGFX_SAMPLER_U_CLAMP
        | BGFX_SAMPLER_V_CLAMP
        | BGFX_SAMPLER_MIN_POINT
        | BGFX_SAMPLER_MAG_POINT
        | (color_space == TextureColorSpace::Srgb ? BGFX_TEXTURE_SRGB : BGFX_TEXTURE_NONE);
...
{
    const MaterialColorSrgb base_color = color_parameter(
        material,
        "base_color",
        MaterialColorSrgb{1.0F, 1.0F, 1.0F, 1.0F});
    const MaterialColorSrgb emissive_color = color_parameter(
        material,
        "emissive_color",
        MaterialColorSrgb{0.0F, 0.0F, 0.0F, 1.0F});
    const float emissive_strength = float_parameter(material, "emissive_strength", 0.0F);
    const float metallic = std::clamp(float_parameter(material, "metallic", 0.0F), 0.0F, 1.0F);
    const float roughness = std::clamp(float_parameter(material, "roughness", 0.5F), 0.0F, 1.0F);
    const float normal_strength = std::max(0.0F, float_parameter(material, "normal_strength", 1.0F));
    const float occlusion_strength = std::clamp(float_parameter(material, "occlusion_strength", 1.0F), 0.0F, 1.0F);
    const float opacity = std::clamp(float_parameter(material, "opacity", base_color.a), 0.0F, 1.0F);
    const float alpha_cutoff = std::clamp(float_parameter(material, "alpha_cutoff", 0.5F), 0.0F, 1.0F);
    const MaterialVec2 uv_tiling = vec2_parameter(material, "uv_tiling", MaterialVec2{1.0F, 1.0F});
    const MaterialVec2 uv_offset = vec2_parameter(material, "uv_offset", MaterialVec2{0.0F, 0.0F});
    const bool double_sided = bool_parameter(material, "double_sided", material.overrides.double_sided);

    GpuPbrMaterialBlock block;
    block.base_color_factor_linear = linear_color_array(base_color);
    if (definition.id == BaseShaderId::TransparentPbr) {
        block.base_color_factor_linear[3] = opacity;
    }
    block.emissive_factor_linear = linear_color_array(emissive_color);
    block.emissive_factor_linear[3] = std::max(0.0F, emissive_strength);
    block.factors = {metallic, roughness, normal_strength, occlusion_strength};
    if (definition.id == BaseShaderId::AlphaCutoutPbr) {
        block.factors[3] = alpha_cutoff;
    }
    if (definition.id == BaseShaderId::TransparentPbr) {
        block.factors[3] = opacity;
    }
    block.uv_transform_0 = {uv_tiling.x, uv_tiling.y, uv_offset.x, uv_offset.y};
    block.flags = {
...
GpuMaterialCache::GpuMaterialCache()
...
bool GpuMaterialCache::initialize(RendererStatus& status)
{
    ...
    impl_->initialized = impl_->base_color_uniform.valid()
        && impl_->emissive_uniform.valid()
        && impl_->factors_uniform.valid()
        && impl_->uv_transform_uniform.valid()
        && impl_->flags_uniform.valid();
