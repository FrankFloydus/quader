...
RenderProgramHandle GpuProgramCache::load_program(
    ...
    RendererStatus& status)
{
    if (const auto cached = handles_by_program_.find(program); cached != handles_by_program_.end()) {
    ...
    const RenderProgramHandle handle = programs_.create(GpuProgramResource{
        .program = UniqueProgramHandle(bgfx_program),
    });
    handles_by_program_.emplace(program, handle);
    return handle;
}
...
{
    const auto found = handles_by_program_.find(program);
    return found == handles_by_program_.end() ? RenderProgramHandle{} : found->second;
}
