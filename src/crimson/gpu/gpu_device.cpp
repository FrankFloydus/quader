...
[[nodiscard]] std::vector<GraphicsBackend> supported_runtime_backends()
{
    ...
    const std::uint8_t count = bgfx::getSupportedRenderers(
...
{
    const std::uint16_t rgba8_caps = caps.formats[bgfx::TextureFormat::RGBA8];
    const std::uint16_t rgba16f_caps = caps.formats[bgfx::TextureFormat::RGBA16F];
    const std::uint16_t r32u_caps = caps.formats[bgfx::TextureFormat::R32U];
    const std::uint16_t d24s8_caps = caps.formats[bgfx::TextureFormat::D24S8];
    const std::uint16_t d32f_caps = caps.formats[bgfx::TextureFormat::D32F];

    return GpuCapabilityInput{
        .instancing = (caps.supported & BGFX_CAPS_INSTANCING) != 0,
        .texture_2d = format_supports(rgba8_caps, BGFX_CAPS_FORMAT_TEXTURE_2D),
        .texture_cube = format_supports(rgba8_caps, BGFX_CAPS_FORMAT_TEXTURE_CUBE),
        .floating_point_render_target = format_supports(rgba16f_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER),
        .render_to_texture = format_supports(rgba8_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER)
            || format_supports(rgba16f_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER),
        .depth_texture = format_supports(d24s8_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER)
            || format_supports(d32f_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER),
        .srgb_texture_sampling = format_supports(rgba8_caps, BGFX_CAPS_FORMAT_TEXTURE_2D_SRGB),
        .srgb_backbuffer = false,
        .manual_srgb_final_conversion = config.display_conversion.allow_manual_final_srgb,
        .integer_picking_target = format_supports(r32u_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER),
        .rgba8_picking_target = format_supports(rgba8_caps, BGFX_CAPS_FORMAT_TEXTURE_FRAMEBUFFER),
...
{
    const FullscreenTriangleUvRange uv = fullscreen_triangle_uv_range(origin);
    return std::array<FullscreenVertex, 3>{
        FullscreenVertex{-1.0F, 0.0F, 0.0F, uv.min_u, uv.min_v},
        FullscreenVertex{ 1.0F, 0.0F, 0.0F, uv.max_u, uv.min_v},
        FullscreenVertex{ 1.0F, 2.0F, 0.0F, uv.max_u, uv.max_v},
    };
}
    ...
    RendererStatus& status)
{
    ...
    resources.fullscreen_vertex_layout
        ...
        .end();
    const std::array<FullscreenVertex, 3> fullscreen_triangle =
        make_fullscreen_triangle(runtime_screen_origin());
    resources.fullscreen_vertex_buffer.reset(bgfx::createVertexBuffer(
        bgfx::copy(fullscreen_triangle.data(), sizeof(FullscreenVertex) * fullscreen_triangle.size()),
        resources.fullscreen_vertex_layout));
    ...
    resources.ready = resources.fullscreen_vertex_buffer.valid()
        && resources.hdr_scene_sampler.valid()
        && resources.tone_mapped_sampler.valid()
        && resources.tone_map_params_uniform.valid()
        && resources.present_params_uniform.valid();
    if (!resources.ready) {
        push_diagnostic(status, RendererDiagnostic{
            .severity = RendererDiagnosticSeverity::Error,
            .code = RendererDiagnosticCode::ResourceCreationFailed,
...
{
    return static_cast<float>(safe_view_dimension(view.rect.width))
        / static_cast<float>(safe_view_dimension(view.rect.height));
}
    ...
    bgfx::FrameBufferHandle framebuffer = BGFX_INVALID_HANDLE)
{
    ...
    const float aspect = static_cast<float>(std::max<std::uint16_t>(1, view.rect.width))
        / static_cast<float>(std::max<std::uint16_t>(1, view.rect.height));
...
[[nodiscard]] bool manual_final_srgb_conversion_enabled(
    ...
    const RendererConfig& config) noexcept
{
    const std::optional<DisplayConversionPath> path = choose_display_conversion_path(
        capability_supported(status, RendererCapability::SrgbBackbuffer),
        config.display_conversion);
    return path == DisplayConversionPath::ManualFinalShader;
}
...
{
    if (!resources.ready
        || !bgfx::isValid(source_texture)
        || !bgfx::isValid(program)
        || !bgfx::isValid(sampler)
        || !bgfx::isValid(params_uniform)) {
        return 0;
    }
    ...
    PbrSubmitMode mode = PbrSubmitMode::Lit)
{
    ...
    for (const PbrDrawPacket& packet : packets) {
        ...
        const bgfx::ProgramHandle program = program_cache.program(program_cache.cached_handle(packet.program));
        if (mesh == nullptr || !mesh->valid() || definition == nullptr || !bgfx::isValid(program)) {
            continue;
        }
...
{
    std::uint32_t draw_calls = 0;
    const bgfx::ProgramHandle program = program_cache.program(overlay_program);
    const std::uint32_t queue_offset = overlay_queue_offset(depth_mode);
    for (std::size_t view_index = 0; view_index < views.size(); ++view_index) {
        const RenderView& view = views[view_index];
        const bgfx::ViewId view_id = overlay_view_id(view_index, queue_offset);
        const std::string view_name = std::string(render_queue_name(render_queue_for_overlay_depth_mode(depth_mode)))
            + ":" + view.debug_name;
        configure_scene_view(view_id, view, view_name, ViewClearMode::None, framebuffer);

        for (const PreparedGridOverlayCommand& grid : bucket.grid_commands) {
            if (grid.grid.view_index != view.view_index) {
                continue;
            }
            draw_calls += overlay_renderer.submit_grid(view_id, view, grid, program);
        }
    }
    return draw_calls;
}
...
bool GpuDevice::initialize(const RendererConfig& config, const NativeSurfaceDescriptor& surface)
{
    ...
    const ShaderTarget shader_target = shader_target_for_backend(*selected_backend_);
    (void)impl_->program_cache.load_program(shader_library, ShaderProgramId::OpaquePbr, shader_target, status_);
    (void)impl_->program_cache.load_program(shader_library, ShaderProgramId::AlphaCutoutPbr, shader_target, status_);
    (void)impl_->program_cache.load_program(shader_library, ShaderProgramId::TransparentPbr, shader_target, status_);
    ...
    const RenderGraph& graph)
{
    ...
    const ViewportExtent target_extent = snapshot.target_extent();
    if (target_extent.width_px != extent_.width_px
        || target_extent.height_px != extent_.height_px
        || target_extent.device_pixel_ratio != extent_.device_pixel_ratio) {
        if (!resize(target_extent)) {
            const RendererStatus current_status = status();
            RendererDiagnostic diagnostic{
                .severity = RendererDiagnosticSeverity::Error,
                .code = RendererDiagnosticCode::ResizeFailed,
                .message = "Crimson failed to resize before rendering.",
            };
            if (!current_status.diagnostics.empty()) {
                diagnostic = current_status.diagnostics.back();
            }
    ...
    timing_stats.cpu_resource_upload_us += elapsed_us_since(upload_start);

    const auto width_px = safe_view_dimension(target_extent.width_px);
    const auto height_px = safe_view_dimension(target_extent.height_px);
    bgfx::setViewRect(kBackgroundView, 0, 0, width_px, height_px);
    bgfx::setViewClear(kBackgroundView, BGFX_CLEAR_NONE, kViewportClearColor, 1.0F, 0);
    bgfx::touch(kBackgroundView);

    const bgfx::FrameBufferHandle hdr_scene_framebuffer = impl_->frame_resources.hdr_scene_framebuffer();
    const bgfx::FrameBufferHandle scene_depth_framebuffer = impl_->frame_resources.scene_depth_framebuffer();
    const bgfx::FrameBufferHandle tone_mapped_color_framebuffer =
        impl_->frame_resources.tone_mapped_color_framebuffer();
    const bgfx::FrameBufferHandle tone_mapped_framebuffer = impl_->frame_resources.tone_mapped_framebuffer();
    const bgfx::TextureHandle hdr_scene_texture = impl_->frame_resources.texture(kHdrSceneColorTargetName);
    const bgfx::TextureHandle tone_mapped_texture = impl_->frame_resources.texture(kToneMappedColorTargetName);
    ...
    if (impl_->prototype_resources.ready) {
        ...
        for (std::size_t index = 0; index < snapshot.views().size(); ++index) {
            ...
            const bgfx::ViewId depth_prepass_view_id = depth_view_id(index);
            configure_scene_view(
                depth_prepass_view_id,
                view,
                "DepthPrepass:" + view.debug_name,
                ViewClearMode::Depth,
                scene_depth_framebuffer);

            const auto packet_build_start = std::chrono::steady_clock::now();
            ...
            timing_stats.cpu_packet_build_us += elapsed_us_since(packet_build_start);
            ...
            const std::vector<DrawPacket> lit_packets = collect_lit_packets(packets);
            const std::vector<InstanceBatch> batches = build_instance_batches(lit_packets);
            accumulate_instancing_stats(instancing_stats, make_instancing_stats(batches));

            const bgfx::ViewId opaque_view_id = scene_view_id(index, 0);
            configure_scene_view(
                opaque_view_id,
                view,
            ...
            const bgfx::ViewId cutout_view_id = scene_view_id(index, 1);
            configure_scene_view(
                cutout_view_id,
                view,
                ...
                ViewClearMode::None,
                hdr_scene_framebuffer);
            const bgfx::ViewId transparent_view_id = scene_view_id(index, 2);
            configure_scene_view(
                transparent_view_id,
                view,
        ...
        const bgfx::ProgramHandle tone_map_program = impl_->program_cache.program(impl_->tone_map_program);
        const std::array<float, 4> tone_map_params = {
        ...
        queue_stats.draw_packet_count += submit_fullscreen_pass(
            ...
            if (snapshot.viewport_settings().draw_overlays
                && snapshot.viewport_settings().draw_grid_overlay
                && impl_->overlay_renderer.ready()) {
                const OverlayDrawLists overlay_draw_lists = OverlaySystem{}.prepare(
                    snapshot.overlays(),
                    snapshot.grid_overlay_payloads());
            const std::uint32_t overlay_packet_count = size_to_u32(overlay_draw_lists.command_count());
            queue_stats.draw_packet_count += overlay_packet_count;
            packet_stats.draw_packet_count += overlay_packet_count;
            packet_stats.overlay_packet_count += overlay_packet_count;
                queue_stats.overlay_draw_count += submit_grid_overlay_bucket(
                overlay_draw_lists.depth_tested,
                OverlayDepthMode::DepthTested,
                snapshot.views(),
                impl_->overlay_renderer,
                impl_->program_cache,
                impl_->overlay_program,
                tone_mapped_framebuffer);
            queue_stats.overlay_draw_count += submit_grid_overlay_bucket(
                overlay_draw_lists.xray,
        ...
        const bgfx::ProgramHandle present_program = impl_->program_cache.program(impl_->present_program);
        const std::array<float, 4> present_params = {
        ...
        queue_stats.draw_packet_count += submit_fullscreen_pass(
            ...
            present_params,
            impl_->post_resources);
        timing_stats.cpu_submit_us += elapsed_us_since(submit_start);
    }

    impl_->completed_bgfx_frame = bgfx::frame();
    timing_stats.cpu_frame_us = elapsed_us_since(frame_start);

    ++impl_->frame_index;
    ++impl_->stats_frame_count;

    const FrameUploadStats current_runtime_upload_stats =
        runtime_upload_stats(impl_->mesh_cache, impl_->material_cache);
    upload_stats = subtract_upload_stats(
        current_runtime_upload_stats,
        impl_->previous_runtime_upload_stats);
    impl_->previous_runtime_upload_stats = current_runtime_upload_stats;

    double fps = impl_->latest_frame_stats ? impl_->latest_frame_stats->fps : 0.0;
    const double elapsed_seconds = snapshot.elapsed_seconds();
    if (elapsed_seconds - impl_->last_stats_seconds >= 1.0) {
        const double delta = elapsed_seconds - impl_->last_stats_seconds;
        fps = delta > 0.0 ? static_cast<double>(impl_->stats_frame_count) / delta : 0.0;
        impl_->last_stats_seconds = elapsed_seconds;
        impl_->stats_frame_count = 0;
    }

    impl_->latest_frame_stats = make_frame_stats(FrameStatsInput{
        .frame_index = impl_->frame_index,
        .width_px = target_extent.width_px,
        .height_px = target_extent.height_px,
