...
RendererDiagnostic make_backend_unsupported_diagnostic(
	...
	return RendererDiagnostic{
		...
		.detail = "Platform=" + std::string(native_surface_platform_name(platform)) + ", requested=" + std::string(graphics_backend_preference_name(preference)),
		.subsystem = "gpu",
...
GpuCapabilityValidation validate_v1_capabilities(
	...
	const bool kSrgbBackbuffer = display_conversion.prefer_srgb_backbuffer && input.srgb_backbuffer;
	push_status(
			validation.statuses,
	...
	const bool kManualSrgb = display_conversion.allow_manual_final_srgb && input.manual_srgb_final_conversion;
	push_status(
			validation.statuses,
	...
	if (!kSrgbBackbuffer && !kManualSrgb) {
