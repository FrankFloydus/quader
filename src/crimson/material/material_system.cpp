...
MaterialSystem::MaterialSystem() : MaterialSystem(make_v1_base_shader_registry()) {
}

MaterialSystem::MaterialSystem(BaseShaderRegistry registry) : registry_(std::move(registry)) {
}
...
quader::foundation::Result<RenderMaterialHandle, RendererDiagnostic> MaterialSystem::create_material(
	...
	const auto kNormalized = normalize(instance);
	if (!kNormalized) {
		return quader::foundation::Result<RenderMaterialHandle, RendererDiagnostic>::failure(kNormalized.error());
	}
	...
	if (!free_indices_.empty()) {
		...
		slot.material = kNormalized.value();
		slot.occupied = true;
	} else {
		index = static_cast<std::uint32_t>(slots_.size() + 1);
		slots_.push_back(Slot{
				.material = kNormalized.value(),
				.generation = 1,
				.occupied = true,
		});
	}
...
quader::foundation::Result<void, RendererDiagnostic> MaterialSystem::set_parameter(
	...
	const auto kNormalized = normalize(candidate);
	if (!kNormalized) {
		return quader::foundation::Result<void, RendererDiagnostic>::failure(kNormalized.error());
	}
...
quader::foundation::Result<MaterialInstance, RendererDiagnostic> MaterialSystem::normalize(
	...
	const auto kValidation = validate_material_instance(registry_, instance);
	if (!kValidation) {
		return quader::foundation::Result<MaterialInstance, RendererDiagnostic>::failure(kValidation.error());
	}
