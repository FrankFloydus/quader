...
std::uint32_t decode_picking_id_rgba8(PackedPickingIdRgba8 encoded) noexcept {
	return static_cast<std::uint32_t>(encoded.bytes[0]) | (static_cast<std::uint32_t>(encoded.bytes[1]) << 8U) | (static_cast<std::uint32_t>(encoded.bytes[2]) << 16U) | (static_cast<std::uint32_t>(encoded.bytes[3]) << 24U);
}
