...
{
    const float sx = std::sin(x_radians);
    const float cx = std::cos(x_radians);
    const float sy = std::sin(y_radians);
    const float cy = std::cos(y_radians);
...
{
    const float animation_time = animation_enabled ? static_cast<float>(elapsed_seconds) : 0.0F;
    RenderObject object;
    object.object_id = 1;
    object.base_shader = BaseShaderId::OpaquePbr;
    object.queue = RenderQueue::Opaque;
    object.world_from_object = rotation_xy(animation_time * 0.8F, animation_time * 1.1F);
