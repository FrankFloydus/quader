    ...
    double device_pixel_ratio)
{
    ...
    const crimson::RendererStatus status = renderer_->status();
    renderer_name_ = QString::fromStdString(status.backend_name);
...
ViewportRenderResult CrimsonViewportRenderHost::render_frame(const ViewportRenderRequest& request)
{
    ...
    const auto cameras = make_cameras(request.cameras);
    const auto views = make_views(request.panes);
    const std::vector<crimson::PickingRequest> picking_requests = make_picking_requests(request.picking_requests);
    const auto frame = crimson::PrototypeViewportFrame{
        .target_extent = make_extent(request.surface_size, request.device_pixel_ratio),
        .views = std::span<const crimson::PrototypeViewportView>(views.data(), request.panes.size()),
        .cameras = std::span<const crimson::PrototypeCamera>(cameras.data(), cameras.size()),
        .picking_requests = std::span<const crimson::PickingRequest>(picking_requests.data(), picking_requests.size()),
        .animation_enabled = request.prototype_animation_enabled,
        .elapsed_seconds = request.elapsed_seconds,
    };

    const crimson::FrameBuilder frame_builder;
    auto snapshot = frame_builder.build_prototype_snapshot(frame);
    if (!snapshot) {
    ...
    const crimson::FrameStats stats = frame_result.stats;
    latest_stats_ = viewport_frame_stats_from(stats);
