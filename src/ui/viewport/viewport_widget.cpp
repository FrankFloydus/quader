...
public:
    explicit StyledSplitterHandle(Qt::Orientation orientation, QWidget* parent = nullptr)
        : QWidget(parent)
...
ViewportWidget::ViewportWidget(ViewportController& controller, QWidget* parent)
    : QWidget(parent)
...
{
    const double dpr = std::max(0.01, devicePixelRatioF());
    return ViewportPixelSize{
        .width = std::max(1, static_cast<int>(std::round(width() * dpr))),
        .height = std::max(1, static_cast<int>(std::round(height() * dpr))),
    };
}
...
void ViewportWidget::wheelEvent(QWheelEvent* event)
{
    ...
    if (std::abs(steps) > 0.000001F) {
        controller_.handle_wheel(steps, viewport_point_from(event->position() * devicePixelRatioF()), pixel_size());
        event->accept();
        return;
    }
...
{
    const bool is_vertical_handle = watched == vertical_splitter_handle_;
    const bool is_horizontal_handle = watched == horizontal_splitter_handle_;

    if (!is_vertical_handle && !is_horizontal_handle) {
        return QWidget::eventFilter(watched, event);
    }

    const ViewportSplitHandle handle = is_vertical_handle
    ...
    case QEvent::MouseMove: {
        auto* mouse_event = static_cast<QMouseEvent*>(event);
        if (active_split_handle_ == handle) {
            set_split_from_position(mapFromGlobal(mouse_event->globalPosition().toPoint()));
            return true;
        }
        break;
    ...
    case QEvent::MouseButtonRelease: {
        auto* mouse_event = static_cast<QMouseEvent*>(event);
        if (mouse_event->button() == Qt::LeftButton && active_split_handle_ == handle) {
            set_split_from_position(mapFromGlobal(mouse_event->globalPosition().toPoint()));
            active_split_handle_ = ViewportSplitHandle::None;
            update_splitter_cursor(mapFromGlobal(QCursor::pos()));
            return true;
...
void ViewportWidget::render_now()
{
    ...
    const double elapsed_seconds = elapsed_.isValid() ? elapsed_.elapsed() / 1000.0 : 0.0;
    const float delta_seconds = static_cast<float>(std::clamp(elapsed_seconds - last_frame_seconds_, 0.0, 0.25));
    last_frame_seconds_ = elapsed_seconds;
    controller_.render_frame(elapsed_seconds, delta_seconds);
}
...
void ViewportWidget::update_splitter_handles()
{
    ...
    const double dpr = std::max(0.01, devicePixelRatioF());
    const ViewportRect vertical = controller_.layout_state().vertical_splitter_rect(pixel_size());
    const ViewportRect horizontal = controller_.layout_state().horizontal_splitter_rect(pixel_size());

    vertical_splitter_handle_->setGeometry(
        static_cast<int>(std::round(vertical.x / dpr)),
        static_cast<int>(std::round(vertical.y / dpr)),
        std::max(1, static_cast<int>(std::round(vertical.width / dpr))),
        std::max(1, static_cast<int>(std::round(vertical.height / dpr))));
    horizontal_splitter_handle_->setGeometry(
        static_cast<int>(std::round(horizontal.x / dpr)),
        static_cast<int>(std::round(horizontal.y / dpr)),
        std::max(1, static_cast<int>(std::round(horizontal.width / dpr))),
        std::max(1, static_cast<int>(std::round(horizontal.height / dpr))));
