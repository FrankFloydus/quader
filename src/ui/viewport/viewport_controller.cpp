...
ViewportController::ViewportController(IViewportRenderHost& render_host, QObject* parent)
    : QObject(parent)
    ...
    double device_pixel_ratio)
{
    ...
    const ViewportRenderResult result = render_host_.initialize_surface(surface, surface_size_, device_pixel_ratio_);
    if (!result.ok) {
    ...
    const QString renderer_name = result.renderer_name.isEmpty() ? render_host_.renderer_name() : result.renderer_name;
    if (!renderer_name.isEmpty()) {
        Q_EMIT renderer_ready(renderer_name);
    }
    request_frame();
}
...
void ViewportController::resize_surface(ViewportPixelSize size, double device_pixel_ratio)
{
    ...
    const ViewportRenderResult result = render_host_.resize_surface(surface_size_, device_pixel_ratio_);
    if (!result.ok) {
        emit_result_error(result);
    }
    request_frame();
...
void ViewportController::render_frame(double elapsed_seconds, float delta_seconds)
{
    ...
    const ViewportPaneArray panes = layout_.panes_for(surface_size_);
    const ViewportCameraSnapshotArray cameras = cameras_.camera_snapshots();
    const int pane_count = layout_.pane_count();
    const ViewportRenderRequest request{
        ...
        .panes = std::span<const ViewportPane>(panes.data(), static_cast<std::size_t>(pane_count)),
        .cameras = std::span<const ViewportCameraSnapshot>(cameras.data(), cameras.size()),
        .prototype_animation_enabled = prototype_animation_enabled_,
        .elapsed_seconds = elapsed_seconds,
    };

    const ViewportRenderResult result = render_host_.render_frame(request);
    if (!result.ok) {
        emit_result_error(result);
        return;
    }

    if (const std::optional<ViewportFrameStats> stats = render_host_.latest_frame_stats()) {
        Q_EMIT frame_stats_changed(*stats);
    }
}
...
void ViewportController::handle_mouse_press(
    ...
    ViewportPixelSize size)
{
    const int pane_index = layout_.pane_at(size, point);
    if (pane_index >= 0) {
        cameras_.set_active_camera_index(pane_index);
    }

    if (button == ViewportMouseButton::Middle) {
        const NavigationMode mode = shift_modifier ? NavigationMode::Pan : NavigationMode::Orbit;
        if (cameras_.begin_navigation(cameras_.active_camera_index(), mode, point)) {
            request_frame();
        }
    } else if (button == ViewportMouseButton::Right) {
        const NavigationMode mode = cameras_.camera_projection(cameras_.active_camera_index())
...
void ViewportController::handle_wheel(float wheel_steps, ViewportPoint point, ViewportPixelSize size)
{
    ...
    const int pane_index = layout_.pane_at(size, point);
    const int camera_index = pane_index >= 0 ? pane_index : cameras_.active_camera_index();
    cameras_.apply_wheel_zoom(camera_index, wheel_steps, pane_size_for_camera(size, camera_index));
    request_frame();
}
...
{
    const ViewportPaneArray panes = layout_.panes_for(size);
    for (int index = 0; index < layout_.pane_count(); ++index) {
        const ViewportPane& pane = panes[static_cast<std::size_t>(index)];
