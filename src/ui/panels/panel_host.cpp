...
PanelHost::PanelHost(QMainWindow &main_window, PanelContext context, QObject *parent) : QObject(parent), main_window_(main_window), context_(context) {
}
...
QDockWidget *PanelHost::dock(PanelId id) const {
	const auto kIt = docks_.find(id);
	return kIt == docks_.end() ? nullptr : kIt->second;
}
...
void PanelHost::restore_workspace(SettingsService &settings) {
	for (const auto &[panel, panel_dock] : docks_) {
		...
		const auto kSpec = specs_.find(panel);
		const bool kFallbackVisible = kSpec == specs_.end() ? !panel_dock->isHidden() : kSpec->second.default_visible;
		const bool kVisible = settings.panel_visible(panel, kFallbackVisible);
		if (kVisible) {
			panel_dock->show();
		} else {
			panel_dock->hide();
