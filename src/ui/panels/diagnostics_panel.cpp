...
DiagnosticsPanel::DiagnosticsPanel(PanelContext context, QWidget* parent)
    : QWidget(parent)
    , context_(context)
{
...
{
    const ViewportDiagnosticsSummary summary = context_.ui.viewport_diagnostics.summary();
    summary_label_->setText(summary.status_text);
    summary_label_->setToolTip(summary.tooltip_text);
...
{
    const QModelIndexList rows = tree_view_->selectionModel()->selectedRows(static_cast<int>(DiagnosticsItemColumn::Name));
    if (rows.isEmpty()) {
...
{
    const QString text = model_->copy_text_for_all();
    if (text.isEmpty()) {
