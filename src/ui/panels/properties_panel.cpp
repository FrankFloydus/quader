...
PropertiesPanel::PropertiesPanel(PanelContext context, QWidget* parent)
    : QWidget(parent)
    , context_(context)
{
...
{
    const bool has_rows = model_ != nullptr && model_->rowCount() > 0;
    empty_label_->setVisible(!has_rows);
    table_view_->setVisible(has_rows);
}
