...
DiagnosticsItemModel::DiagnosticsItemModel(ViewportDiagnosticsService& diagnostics, QObject* parent)
...
QModelIndex DiagnosticsItemModel::index(int row, int column, const QModelIndex& parent_index) const
{
    ...
    if (row_index >= siblings.size()) {
        return {};
    }

    return createIndex(row, column, siblings[row_index].get());
}
...
QVariant DiagnosticsItemModel::data(const QModelIndex& model_index, int role) const
{
    ...
    case DiagnosticsItemRoles::NodeKindRole:
        return QVariant::fromValue(node->kind);
    case DiagnosticsItemRoles::SeverityRole:
        return node->severity;
    case DiagnosticsItemRoles::CategoryRole:
        return node->category;
    case DiagnosticsItemRoles::FrameIndexRole:
        return node->frame_index.has_value() ? QVariant::fromValue(*node->frame_index) : QVariant{};
    case DiagnosticsItemRoles::CopyTextRole:
