...
[[nodiscard]] PropertyDescriptor make_selection_summary(QString value) {
...
build_property_descriptors(const quader::document::Document &document) {
  ...
  const auto object_id = selection.selected_objects().front();
  ...
  descriptors.push_back(make_descriptor(
      object_id, PropertyField::ObjectName, QStringLiteral("Name"),
      PropertyValueKind::String, QString::fromStdString(object->name), true,
      QStringLiteral("Object name")));
  descriptors.push_back(make_descriptor(
      object_id, PropertyField::ObjectKind, QStringLiteral("Kind"),
      PropertyValueKind::ReadOnlyText, QStringLiteral("Mesh")));
  descriptors.push_back(make_descriptor(
      object_id, PropertyField::VertexCount, QStringLiteral("Vertices"),
      PropertyValueKind::Int, count_value(object->mesh.vertex_count())));
  descriptors.push_back(make_descriptor(
      object_id, PropertyField::EdgeCount, QStringLiteral("Edges"),
      PropertyValueKind::Int, count_value(object->mesh.edge_count())));
  descriptors.push_back(make_descriptor(
      object_id, PropertyField::FaceCount, QStringLiteral("Faces"),
      PropertyValueKind::Int, count_value(object->mesh.face_count())));

  append_transform_descriptors(descriptors, object_id, object->transform);
  return descriptors;
}
