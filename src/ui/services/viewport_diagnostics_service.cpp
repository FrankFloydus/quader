...
ViewportDiagnosticsService::ViewportDiagnosticsService(QObject* parent)
    : QObject(parent)
{
}
...
ViewportDiagnosticsSummary ViewportDiagnosticsService::summary() const
{
    ...
    QStringList tooltip;
    tooltip.push_back(QStringLiteral("Renderer: %1").arg(
        result.renderer_name.isEmpty() ? QStringLiteral("unknown") : result.renderer_name));
...
{
    const bool had_snapshot = latest_snapshot_.has_value();
