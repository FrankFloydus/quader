...
{
    const auto normalized = quader::io::normalize_extension(extension);
    if (normalized.empty()) {
        return {};
    }

    return QStringLiteral("*.%1").arg(QString::fromStdString(normalized));
}
...
[[nodiscard]] QString first_warning_detail(const quader::io::IoDiagnosticList& diagnostics)
{
    for (const auto& diagnostic : diagnostics) {
        if (diagnostic.severity == quader::io::IoSeverity::Warning
...
ImportUiResult ImportUiController::open_scene(QWidget* parent)
{
    ...
    const auto selected = dialogs_.choose_open_file(parent, make_open_request());
    if (!selected.has_value()) {
        return ImportUiResult::Canceled;
    }

    const std::filesystem::path selected_path = path_from_qstring(*selected);
    auto imported = import_service_.import_file(quader::io::ImportRequest{selected_path, {}});
    if (!imported) {
    ...
    present_parsed_not_applied(selected_path);
...
{
    const QString detail = first_warning_detail(diagnostics);
    if (detail.isEmpty()) {
        return;
    }
    ...
    const std::vector<quader::io::FileFormatDescriptor>& formats)
{
    ...
    for (const auto& format : formats) {
        ...
        for (const auto& extension : format.extensions) {
            const QString pattern = pattern_for_extension(extension);
            append_unique_pattern(patterns, pattern);
            append_unique_pattern(all_patterns, pattern);
        }
