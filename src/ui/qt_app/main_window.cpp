...
MainWindow::MainWindow(UiContext& context, QWidget* parent)
    : QMainWindow(parent)
    , context_(context)
{
...
void MainWindow::connect_notifications()
{
    connect(&context_.notifications,
            ...
            [this](const NotificationMessage& message) {
                const QString text = notification_text(message);
                const int timeout = message.timeout_ms < 0 ? 0 : message.timeout_ms;
...
{
    const QByteArray geometry = context_.settings.main_window_geometry();
    if (!geometry.isEmpty()) {
        restoreGeometry(geometry);
    }

    const QByteArray state = context_.settings.main_window_state();
    bool state_restored = false;
    if (!state.isEmpty()) {
        state_restored = restoreState(state, SettingsService::kWorkspaceStateVersion);
    }
...
void MainWindow::update_diagnostics_label()
{
    ...
    const ViewportDiagnosticsSummary summary = context_.viewport_diagnostics.summary();
    diagnostics_label_->setText(summary.status_text);
    diagnostics_label_->setToolTip(summary.tooltip_text);
}
