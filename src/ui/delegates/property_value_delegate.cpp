...
[[nodiscard]] PropertyValueKind value_kind_for_index(const QModelIndex &index) {
	return index.data(property_item_roles::kValueKindRole)
			.value<PropertyValueKind>();
}

[[nodiscard]] PropertyDescriptor
descriptor_for_index(const QModelIndex &index) {
	return index.data(property_item_roles::kDescriptorRole)
			.value<PropertyDescriptor>();
}

[[nodiscard]] bool editable_for_index(const QModelIndex &index) {
	return index.data(property_item_roles::kEditableRole).toBool();
}

} // namespace

PropertyValueDelegate::PropertyValueDelegate(QObject *parent) : QStyledItemDelegate(parent) {}
...
QWidget *PropertyValueDelegate::createEditor(QWidget *parent,
	...
	switch (value_kind_for_index(index)) {
		...
		case PropertyValueKind::Float: {
			const auto kDescriptor = descriptor_for_index(index);
			auto *editor = new QDoubleSpinBox(parent);
			editor->setFrame(false);
			editor->setKeyboardTracking(false);
			editor->setDecimals(kDescriptor.decimals);
			editor->setMinimum(kDescriptor.minimum.isValid()
							? kDescriptor.minimum.toDouble()
							: -std::numeric_limits<double>::max());
			editor->setMaximum(kDescriptor.maximum.isValid()
							? kDescriptor.maximum.toDouble()
							: std::numeric_limits<double>::max());
			return editor;
		}
...
QString PropertyValueDelegate::displayText(const QVariant &value,
	...
	if (value.metaType().id() == QMetaType::Double ||
		...
		bool ok = false;
		const double kNumber = value.toDouble(&ok);
		if (ok) {
			return locale.toString(kNumber, 'f', 3);
		}
	}
