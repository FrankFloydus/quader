...
void ActionStateUpdater::refresh_from_snapshot(const EditorStateSnapshot& snapshot)
{
    ...
    const bool tools_enabled = snapshot.tools_available;
    set_enabled(actions_, ActionId::SelectTool, tools_enabled);
    set_enabled(actions_, ActionId::MoveTool, tools_enabled);
    set_enabled(actions_, ActionId::RotateTool, tools_enabled);
    set_enabled(actions_, ActionId::ScaleTool, tools_enabled);

    set_checked(actions_, ActionId::SelectTool, tools_enabled && snapshot.active_tool == ActionId::SelectTool);
    set_checked(actions_, ActionId::MoveTool, tools_enabled && snapshot.active_tool == ActionId::MoveTool);
    set_checked(actions_, ActionId::RotateTool, tools_enabled && snapshot.active_tool == ActionId::RotateTool);
    set_checked(actions_, ActionId::ScaleTool, tools_enabled && snapshot.active_tool == ActionId::ScaleTool);

    const bool creation_enabled = snapshot.has_active_document && snapshot.creation_available;
    set_enabled(actions_, ActionId::CreateCube, creation_enabled);
    set_enabled(actions_, ActionId::CreateLight, creation_enabled);
    set_enabled(actions_, ActionId::CreateCamera, creation_enabled);
