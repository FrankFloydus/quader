...
{
    return tool_manager.has_tool(quader::tools::ToolId::Select)
        || tool_manager.has_tool(quader::tools::ToolId::Move)
        || tool_manager.has_tool(quader::tools::ToolId::Rotate)
        || tool_manager.has_tool(quader::tools::ToolId::Scale);
}
...
EditorStateSnapshot EditorActionStateProvider::editor_state_snapshot() const
{
    ...
    snapshot.can_delete_selection = document_.selection().selected_objects().size() == 1U
        && document_.selection().selected_components().empty();
    ...
    if (const auto active_tool = tool_manager_.active_tool_id()) {
        snapshot.active_tool = action_for_tool(*active_tool);
    }
