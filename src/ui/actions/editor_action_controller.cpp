...
void EditorActionController::handle_action_toggled(ActionId id, bool checked)
{
    ...
    const auto tool = tool_for_action(id);
    if (!tool.has_value()) {
        return;
    }

    if (!tool_manager_.set_active_tool(*tool)) {
...
void EditorActionController::delete_selection()
{
    ...
    const auto selected_objects = document.selection().selected_objects();
    if (selected_objects.size() != 1U || !document.selection().selected_components().empty()) {
        notifications_.show_warning(NotificationMessage{
            NotificationSeverity::Warning,
    ...
    (void)document_ui_.execute_command(
        std::make_unique<quader::commands::DeleteObjectCommand>(selected_objects.front()));
}
