...
QAction& ActionRegistry::register_action(ActionId id, ActionSpec spec)
{
    if (auto existing = actions_.find(id); existing != actions_.end()) {
        return *existing->second;
    }

    auto* action = new QAction(std::move(spec.text), this);
    action->setStatusTip(std::move(spec.status_tip));
