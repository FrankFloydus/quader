...
quader::foundation::Result<void, DocumentError> Document::restore_mesh_object(MeshObject object)
{
    ...
    emit_change(DocumentChangeKind::ObjectCreated, id);
    mark_dirty(to_dirty_flags(DocumentDirtyFlag::Objects));
...
quader::foundation::Result<MeshObject, DocumentError> Document::extract_mesh_object(ObjectId id)
{
    ...
    const bool selection_changed = selection_.references_object(id);
    ...
    const quader::mesh::Polyhedron& mesh) const
{
    ...
    if (!report.issues().empty()) {
        message += ": ";
        message += report.issues().front().diagnostic.message;
    }
...
{
    for (const auto object : selection.selected_objects()) {
        auto validation = validate_object_id(object);
        if (!validation) {
            return validation;
        }
    }
...
{
    const auto new_flags = dirty_flags_ | flags;
    if (new_flags == dirty_flags_) {
        return;
    }
