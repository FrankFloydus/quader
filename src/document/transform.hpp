...
{
    return vec3_exactly_equal(left.translation, right.translation)
        && vec3_exactly_equal(left.rotation_euler, right.rotation_euler)
        && vec3_exactly_equal(left.scale, right.scale);
}

[[nodiscard]] inline bool is_finite(Transform transform) noexcept
{
    return quader::math::is_finite(transform.translation)
        && quader::math::is_finite(transform.rotation_euler)
        && quader::math::is_finite(transform.scale);
}
