...
struct hash<quader::foundation::GenerationalId<Tag>> {
    ...
    {
        const auto index_hash = hash<quader::foundation::IdIndex>{}(id.index());
        const auto generation_hash = hash<quader::foundation::IdGeneration>{}(id.generation());
        return index_hash ^ (generation_hash + 0x9e3779b9U + (index_hash << 6U) + (index_hash >> 2U));
    }
};
