...
struct AttributeValueTraits<float> {
    ...
    static constexpr AttributeValueType value_type = AttributeValueType::Float32;
};

template <>
struct AttributeValueTraits<quader::math::Vec2> {
    static constexpr bool supported = true;
    static constexpr AttributeValueType value_type = AttributeValueType::Vec2Float32;
};

template <>
struct AttributeValueTraits<quader::math::Vec3> {
    static constexpr bool supported = true;
    static constexpr AttributeValueType value_type = AttributeValueType::Vec3Float32;
};

template <>
struct AttributeValueTraits<std::uint32_t> {
    static constexpr bool supported = true;
    static constexpr AttributeValueType value_type = AttributeValueType::UInt32;
};
...
public:
    ...
    MeshAttributes(const MeshAttributes& other)
    ...
    [[nodiscard]] quader::foundation::Result<AttributeId, MeshError> create(AttributeDomain domain,
                                                                            ...
                                                                            T default_value)
    {
        static_assert(AttributeValueTraits<T>::supported, "Unsupported mesh attribute value type.");
        ...
        const AttributeId id{static_cast<quader::foundation::IdIndex>(storages_.size())};
        auto storage = std::make_unique<TypedAttributeStorage<T>>(
            AttributeDescriptor{id, domain, AttributeValueTraits<T>::value_type, std::move(name)},
            std::move(default_value),
            domain_size(domain));
        storages_.push_back(std::move(storage));

        return quader::foundation::Result<AttributeId, MeshError>::success(id);
    }
...
private:
    ...
    public:
        TypedAttributeStorage(AttributeDescriptor descriptor, T default_value, std::size_t size)
            : AttributeStorage(std::move(descriptor))
            , default_value_(std::move(default_value))
            , values_(size, default_value_)
        {
        }
