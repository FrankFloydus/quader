...
struct UiFixture {
    ...
    UiFixture()
        : settings_store(settings_dir.path() + QStringLiteral("/settings.ini"), QSettings::IniFormat)
        , import_service(io_registry)
        , action_state_updater(actions, editor_state)
        , settings(settings_store)
        , document_ui(document, command_history, action_state_updater, notifications)
        , import_ui(file_dialogs, io_registry, import_service, notifications)
        , context{
              actions,
...
TEST(UiShell, ActionRegistryRegistersCanonicalStandardActions)
{
    ...
    for (const quader::ui::ActionId id : kStandardActions) {
        EXPECT_TRUE(registry.contains(id));
        EXPECT_NE(quader::ui::action_id_name(id), std::string_view("Unknown"));
    }
...
TEST(UiShell, SettingsServiceUsesWorkspaceKeysAndResetScope)
{
    ...
    const QByteArray geometry("geometry");
    const QByteArray state("state");
    settings.set_main_window_geometry(geometry);
    settings.set_main_window_state(state);
    ...
    EXPECT_EQ(settings.main_window_geometry(), geometry);
    EXPECT_EQ(settings.main_window_state(), state);
