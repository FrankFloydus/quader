...
{
    const QList<quader::ui::FileDialogFilter> filters = quader::ui::import_filters_from_formats({
        gltf_import_format(),
        fake_import_format(),
    });

    EXPECT_EQ(filters.size(), 3);
    EXPECT_EQ(filters[0].label, QStringLiteral("All Supported Files"));
    EXPECT_EQ(filters[0].patterns.size(), 3);
    EXPECT_EQ(filters[0].patterns[0], QStringLiteral("*.gltf"));
    EXPECT_EQ(filters[0].patterns[1], QStringLiteral("*.glb"));
    EXPECT_EQ(filters[0].patterns[2], QStringLiteral("*.fake"));
    EXPECT_EQ(filters[1].label, QStringLiteral("glTF 2.0"));
    EXPECT_EQ(filters[1].patterns[0], QStringLiteral("*.gltf"));
    EXPECT_EQ(filters[1].patterns[1], QStringLiteral("*.glb"));
    EXPECT_EQ(filters[2].label, QStringLiteral("Fake Scene"));
    EXPECT_EQ(filters[2].patterns[0], QStringLiteral("*.fake"));
}
...
TEST(UiImportController, NoRegisteredImporterReportsWarningWithoutOpeningDialog)
{
    ...
    const auto result = controller.open_scene(nullptr);

    EXPECT_EQ(result, quader::ui::ImportUiResult::NoImporterRegistered);
...
TEST(UiImportController, CanceledOpenDialogDoesNotCallImporterOrPostFailure)
{
    ...
    EXPECT_EQ(result, quader::ui::ImportUiResult::Canceled);
...
TEST(UiImportController, UnsupportedFormatPresentsDiagnosticWithoutImporterExecution)
{
    ...
    EXPECT_EQ(result, quader::ui::ImportUiResult::ImportFailed);
    EXPECT_EQ(dialogs.open_calls, 1);
    EXPECT_EQ(importer_ptr->import_calls, 0);
    EXPECT_EQ(capture.messages.size(), 1U);
    EXPECT_EQ(capture.messages.front().severity, quader::ui::NotificationSeverity::Warning);
