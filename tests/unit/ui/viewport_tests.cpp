...
TEST(Viewport, LayoutStateClampsQuadPanesAndHitTests)
{
    ...
    const quader::ui::ViewportPaneArray single = layout.panes_for({800, 600});
    EXPECT_EQ(single[0].rect.width, 800);
    EXPECT_EQ(single[0].rect.height, 600);
    ...
    const quader::ui::ViewportPaneArray quad = layout.panes_for({320, 240});
    EXPECT_EQ(layout.pane_count(), 4);
    for (int index = 0; index < 4; ++index) {
        EXPECT_TRUE(quad[static_cast<std::size_t>(index)].rect.width > 0);
        EXPECT_TRUE(quad[static_cast<std::size_t>(index)].rect.height > 0);
    }
...
TEST(Viewport, CameraControllerUpdatesFromSyntheticInputWithoutQtEvents)
{
    ...
    const quader::ui::ViewportCameraSnapshot before = cameras.camera_snapshots()[0];
    ...
    const quader::ui::ViewportCameraSnapshot after = cameras.camera_snapshots()[0];
    EXPECT_TRUE(!quader::math::nearly_equal(before.eye, after.eye, 0.0001F));
    EXPECT_EQ(cameras.navigation_mode(), quader::ui::NavigationMode::None);

    const quader::ui::ViewportCameraSnapshot top = cameras.camera_snapshots()[1];
    EXPECT_EQ(top.projection, quader::ui::CameraProjection::Orthographic);
}
...
TEST(Viewport, RenderResultCarriesTypedPickResultsWithoutSelectionMutation)
{
    ...
    const quader::ui::ViewportRenderResult result = quader::ui::ViewportRenderResult::success(
        QStringLiteral("FakeRenderer"),
        std::move(pick_results));
    EXPECT_TRUE(result.ok);
    EXPECT_EQ(result.completed_pick_results.size(), 1U);
    EXPECT_EQ(result.completed_pick_results[0].request_id, 9U);
    EXPECT_TRUE(result.completed_pick_results[0].hit);
    EXPECT_EQ(result.completed_pick_results[0].payload.object_id, 42U);
}
...
TEST(Viewport, RenderHostCanExposeDiagnosticsSnapshotWithoutGpuHandles)
{
    ...
    const std::optional<quader::ui::ViewportDiagnosticsSnapshot> diagnostics = host.latest_diagnostics();
    EXPECT_TRUE(diagnostics.has_value());
    EXPECT_EQ(diagnostics->renderer_name, QStringLiteral("FakeRenderer"));
    EXPECT_EQ(diagnostics->frame.width, 320);
    EXPECT_EQ(diagnostics->passes.size(), 1);
    EXPECT_EQ(diagnostics->counters.size(), 1);
    EXPECT_EQ(diagnostics->diagnostics.size(), 1);
    EXPECT_EQ(diagnostics->diagnostics[0].resource_name, QStringLiteral("GoldenCapturePass"));
}
...
TEST(Viewport, CrimsonDiagnosticsMappingPreservesStructuredFields)
{
    ...
    const quader::ui::ViewportDiagnosticsSnapshot mapped =
        quader::ui::viewport_diagnostics_from_crimson(source);

    EXPECT_EQ(mapped.renderer_name, QStringLiteral("SyntheticBackend"));
    EXPECT_EQ(mapped.frame.width, 1920);
    EXPECT_EQ(mapped.frame.height, 1080);
    EXPECT_EQ(mapped.frame.viewport_count, 4);
    EXPECT_EQ(mapped.passes.size(), 1);
    EXPECT_EQ(mapped.passes[0].pass_name, QStringLiteral("OpaquePbrPass"));
    EXPECT_EQ(mapped.passes[0].resource_read_count, 2);
    EXPECT_EQ(mapped.passes[0].resource_write_count, 1);
    EXPECT_EQ(mapped.passes[0].draw_call_count, 3);
    EXPECT_EQ(mapped.passes[0].draw_packet_count, 5);
    EXPECT_EQ(mapped.passes[0].cpu_time_us, 17ULL);
    EXPECT_EQ(mapped.counters.size(), 1);
    EXPECT_EQ(mapped.counters[0].domain, QStringLiteral("Upload"));
    EXPECT_EQ(mapped.counters[0].name, QStringLiteral("uploaded_vertex_bytes"));
    EXPECT_EQ(mapped.counters[0].value, 128ULL);
    EXPECT_EQ(mapped.counters[0].unit, QStringLiteral("Bytes"));
    EXPECT_EQ(mapped.diagnostics.size(), 1);
    EXPECT_EQ(mapped.diagnostics[0].severity, QStringLiteral("Warning"));
    EXPECT_EQ(mapped.diagnostics[0].code, QStringLiteral("GoldenCaptureUnsupported"));
    EXPECT_EQ(mapped.diagnostics[0].subsystem, QStringLiteral("golden"));
    EXPECT_EQ(mapped.diagnostics[0].resource_name, QStringLiteral("GoldenCapturePass"));
    EXPECT_EQ(mapped.diagnostics[0].frame_index, 11ULL);
    EXPECT_EQ(mapped.frame_graph_dump, QStringLiteral("FrameSetupPass -> OpaquePbrPass -> PresentPass"));
}
