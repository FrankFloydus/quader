...
TEST(BackendCapability, FullCapabilitySetPassesWithManualSrgbFallback) {
	const crimson::DisplayConversionSettings kDisplay;
	const crimson::gpu::GpuCapabilityValidation kValidation =
			crimson::gpu::validate_v1_capabilities(full_capabilities(), kDisplay);

	expect_true(kValidation.ok(), "full V1 capability set passes validation");
	expect_true(kValidation.statuses.size() == 12, "validation reports every public capability");

	const crimson::RendererCapabilityStatus *srgb_backbuffer = find_status(
			kValidation.statuses,
...
TEST(BackendCapability, MissingRequiredCapabilityReportsFatalDiagnostic) {
	...
	expect_true(!kValidation.ok(), "missing instancing fails V1 capability validation");
	expect_true(!kValidation.diagnostics.empty(), "missing capability produces a diagnostic");
	if (!kValidation.diagnostics.empty()) {
		const crimson::RendererDiagnostic &diagnostic = kValidation.diagnostics.front();
		expect_true(
...
TEST(BackendCapability, FallbackGroupsRequireOneSupportedPath) {
	...
	const crimson::gpu::GpuCapabilityValidation kDisplayValidation =
			crimson::gpu::validate_v1_capabilities(no_display_path, no_manual_display);
	expect_true(!kDisplayValidation.ok(), "display conversion requires sRGB backbuffer or manual fallback");
	...
	const crimson::gpu::GpuCapabilityValidation kPickingValidation =
			crimson::gpu::validate_v1_capabilities(no_picking_target, crimson::DisplayConversionSettings{});
	expect_true(!kPickingValidation.ok(), "picking requires R32U or RGBA8 target fallback");
}
