...
TEST(RenderGraph, ResourceRegistryResizeUpdatesGenerations)
{
    ...
    const std::uint64_t generation_before = before == nullptr ? 0 : before->generation;
    ...
    expect_true(after != nullptr && after->generation != generation_before, "resize advances resource generation");
...
{
    const crimson::RenderGraph graph = crimson::make_minimal_prototype_render_graph(
    ...
    expect_true(graph.debug_dump() == expected, "minimal graph debug dump is stable");
}
