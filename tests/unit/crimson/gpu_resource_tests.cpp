...
TEST(GpuResource, MoveTransfersOwnershipWithoutDoubleDestroy)
{
    FakeHandleTraits::reset_counts();
    {
        ...
        expect_true(!first.valid(), "moved-from handle is invalid");
...
TEST(GpuResource, ResetAndReleaseHaveExplicitLifetimeBehavior)
{
    FakeHandleTraits::reset_counts();
    {
        ...
        const FakeHandle released = handle.release();
        expect_true(released.id == 22, "release returns the owned handle");
...
TEST(GpuResource, ResourceTableAcceptsMoveOnlyRaiiResources)
{
    ...
    const crimson::RenderMeshHandle handle = table.create(MoveOnlyResource{31});
    expect_true(crimson::is_valid_handle(handle), "resource table creates a valid public handle");
    expect_true(table.live_count() == 1, "resource table counts move-only resource as live");
    expect_true(table.get(handle) != nullptr, "resource table resolves move-only resource");

    expect_true(table.destroy(handle), "destroying move-only table resource succeeds");
    ...
    expect_true(table.get(handle) == nullptr, "destroyed public handle no longer resolves");
}
