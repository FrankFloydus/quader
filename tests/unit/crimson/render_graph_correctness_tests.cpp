...
TEST(RenderGraphCorrectness, Task9GraphDeclaresStablePassOrder)
{
    ...
    expect_true(graph.validate().has_value(), "task #9 V1 correctness graph validates");
    expect_true(graph.passes().size() == expected.size(), "task #9 graph declares twelve passes");
    for (std::size_t index = 0; index < expected.size(); ++index) {
        expect_true(graph.passes()[index].name == expected[index], "task #9 graph pass order is stable");
    }
}
...
TEST(RenderGraphCorrectness, Task9GraphDeclaresExpectedFrameTargets)
{
    ...
    const crimson::RenderResourceRecord* hdr = graph.resources().find(crimson::kHdrSceneColorTargetName);
    ...
    const crimson::RenderResourceRecord* tone = graph.resources().find(crimson::kToneMappedColorTargetName);
    ...
    const crimson::RenderResourceRecord* picking = graph.resources().find(crimson::kPickingIdTargetName);
    ...
    const crimson::RenderGraph rgba8_graph = crimson::make_v1_correctness_render_graph(
        crimson::ViewportExtent{320, 240, 1.0F},
        crimson::PickingIdTargetFormat::Rgba8Data);
    const crimson::RenderResourceRecord* rgba8_picking = rgba8_graph.resources().find(crimson::kPickingIdTargetName);
    expect_true(
...
TEST(RenderGraphCorrectness, ToneMapOverlayAndPresentResourceIsolationIsExplicit)
{
    ...
    const crimson::RenderPass* depth_prepass = find_pass(graph, "DepthPrepass");
    expect_true(depth_prepass != nullptr, "depth prepass exists");
    expect_true(
        depth_prepass != nullptr
            && depth_prepass->resources.size() == 1
            && find_use(*depth_prepass, crimson::kSceneDepthTargetName) != nullptr
            && find_use(*depth_prepass, crimson::kHdrSceneColorTargetName) == nullptr,
        "depth prepass writes SceneDepth without touching HDR scene color");

    const crimson::RenderPass* opaque_pbr = find_pass(graph, "OpaquePbrPass");
    ...
    for (std::string_view pass_name : {
        ...
        const crimson::RenderPass* overlay = find_pass(graph, pass_name);
...
TEST(RenderGraphCorrectness, ResizeUpdatesV1Targets)
{
    ...
    const std::uint64_t before_generation = before == nullptr ? 0 : before->generation;
    ...
    expect_true(after != nullptr && after->generation != before_generation, "resize updates target generation");
