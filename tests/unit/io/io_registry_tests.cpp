...
public:
	explicit FakeExporter(quader::io::FileFormatDescriptor descriptor) : descriptor_(std::move(descriptor)) {
	}
...
TEST(IoRegistry, RegistryPreservesRegistrationOrderAndMatchesExtensionsCaseInsensitively) {
	...
	const auto kFormats = registry.import_formats();
	EXPECT_EQ(kFormats.size(), 2U);
	EXPECT_EQ(kFormats[0].id, std::string("first"));
	EXPECT_EQ(kFormats[1].id, std::string("second"));
...
TEST(IoRegistry, RegistryRejectsDuplicateImporterIdsAndExtensions) {
	...
	const auto kDuplicateId = registry.register_importer(
			std::make_unique<FakeImporter>(importer_descriptor("mesh", "other")));
	EXPECT_FALSE(kDuplicateId);
	EXPECT_EQ(kDuplicateId.error().code, quader::io::IoErrorCode::ValidationFailed);

	const auto kDuplicateExtension = registry.register_importer(
			std::make_unique<FakeImporter>(importer_descriptor("other", "MESH")));
	EXPECT_FALSE(kDuplicateExtension);
	EXPECT_EQ(kDuplicateExtension.error().diagnostic.code,
...
TEST(IoRegistry, RegistryRejectsNullAndDuplicateExporterRegistrations) {
	...
	const auto kNullResult = registry.register_exporter(std::move(null_exporter));
	EXPECT_FALSE(kNullResult);

	EXPECT_TRUE(registry.register_exporter(
			std::make_unique<FakeExporter>(exporter_descriptor("scene", "qscene"))));
	const auto kDuplicate = registry.register_exporter(
			std::make_unique<FakeExporter>(exporter_descriptor("other", ".QSCENE")));
	EXPECT_FALSE(kDuplicate);

	const auto kFormats = registry.export_formats();
	EXPECT_EQ(kFormats.size(), 1U);
	EXPECT_EQ(kFormats[0].id, std::string("scene"));
}
