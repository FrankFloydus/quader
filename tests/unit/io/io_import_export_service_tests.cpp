...
public:
	ServiceFakeImporter(quader::io::FileFormatDescriptor descriptor, quader::io::DocumentFragment fragment) : descriptor_(std::move(descriptor)), fragment_(std::move(fragment)) {
	}
...
public:
	explicit ServiceFakeExporter(quader::io::FileFormatDescriptor descriptor) : descriptor_(std::move(descriptor)) {
	}
...
TEST(IoImportExportService, ImportServiceRejectsUnsupportedExtensionsBeforeImporterExecution) {
	...
	const auto kResult = service.import_file(quader::io::ImportRequest{ "scene.unknown", {} });
	EXPECT_FALSE(kResult);
	EXPECT_EQ(kResult.error().code, quader::io::IoErrorCode::UnsupportedFormat);
...
TEST(IoImportExportService, ImportServiceValidatesSuccessfulPayloadsAndPreservesDiagnostics) {
	...
	quader::io::ImportService service{ registry };
	const auto kResult = service.import_file(quader::io::ImportRequest{ "scene.FAKE", {} });
	EXPECT_TRUE(kResult);
	EXPECT_EQ(importer_ptr->import_calls, 1);
	EXPECT_FALSE(kResult.value().diagnostics.has_errors());
	EXPECT_EQ(kResult.value().diagnostics.size(), 1U);
}
...
TEST(IoImportExportService, ImportServiceRejectsInvalidMeshPayloads) {
	...
	EXPECT_FALSE(kResult);
	EXPECT_EQ(importer_ptr->import_calls, 1);
	EXPECT_EQ(kResult.error().code, quader::io::IoErrorCode::ValidationFailed);
	EXPECT_TRUE(kResult.error().related.has_errors());
}
...
TEST(IoImportExportService, ExportServiceRejectsUnsupportedExtensionsBeforeExporterExecution) {
	...
	const auto kResult = service.export_file(quader::io::ExportRequest{ "scene.unknown", document, {} });
	EXPECT_FALSE(kResult);
	EXPECT_EQ(kResult.error().code, quader::io::IoErrorCode::UnsupportedFormat);
...
TEST(IoImportExportService, ExportServiceUsesRegisteredExporterAndDeterministicDefault) {
	...
	quader::document::Document document;
	quader::io::ExportService service{ registry };
	const auto kResult = service.export_file(quader::io::ExportRequest{ "scene.OUT", document, {} });
	EXPECT_TRUE(kResult);
